...
pub const InitFlags = packed struct(c_uint) {
    ...
    /// Only applies to polling-based output modes such as `OutputType.no_sound` or `OutputType.wav_writer`.
    ...
    clip_output: bool,
    _padding_1: u4 = 0,
    /// Enable use of `ChannelControl.setLowPassGain`, `ChannelControl.set3DOcclusion`, or
    /// automatic usage by the `Geometry` API. All voices will add a software low-pass filter
    /// effect into the DSP chain, which is idle unless one of the mentioned functions/features
    /// is used.
    channel_low_pass: bool,
    /// If enabled, all 3D voices will add a software low-pass and high-pass filter effect into the DSP chain,
    /// which will act as a distance-automated band-pass filter. Use `setAdvancedSettings` to adjust
    /// the center frequency.
    channel_distance_filter: bool,
    _padding_2: u6 = 0,
    /// Enable TCP/IP-based host, which allows FMOD Studio or FMOD Profile to connect to this `System`,
    /// and view memory, CPU, and the DSP network graph in real-time.
    profile_enable: bool,
    /// When enabled, any sounds with a volume of 0 will become virtual and will not be processed,
    /// except for having their positions updated virtually. Use `setAdvancedSettings` to adjust
    /// what volume besides zero to switch to virtual at. Implied by `profile_meter_all`.
    vol0_becomes_virtual: bool,
    /// With the geometry engine, only procest the closest polygon, rather than accumulating
    /// all polygons the sound to listener line intersects.
    geometry_use_closest: bool,
    /// When using `SpeakerMode.s5_1` with a stereo output device, use the Dolby Pro Logic II
    /// downmix algorithm, instead of the default stereo downmix algorithm.
    prefer_dolby_downmix: bool,
    /// Disables thread-safety for API calls. Only use this is FMOD is being called from a single
    /// thread, and if the Studio API will not be used!
    thread_unsafe: bool,
    /// Reduces performance. Adds level metering for every single DSP unit in the graph. Use
    /// `DSP.setMeteringEnabled` to turn meters off individually. Setting this flag implies
    /// `profile_enable`.
    profile_meter_all: bool,
    /// Enables memory allocation tracking. Currently only useful when using the Studio API.
    /// Increases memory footprint, and reduces performance. This flag is implied by `studio.System.InitFlags.memory_tracking`.
    ...
    test "core: ensure InitFlags works right" {
        ...
        try expectEqual(log2(0x100), @bitOffsetOf(InitFlags, "channel_low_pass"));
        try expectEqual(log2(0x10000), @bitOffsetOf(InitFlags, "profile_enable"));
        try expectEqual(log2(0x100000), @bitOffsetOf(InitFlags, "thread_unsafe"));
        try expectEqual(log2(0x400000), @bitOffsetOf(InitFlags, "memory_tracking"));
    }
};
...
pub const OutputType = enum(c.FMOD_OUTPUTTYPE) {
    ...
    ///
    /// # Extra Driver Data
    /// Pass the application window handle, as a `*anyopaque`, in the `extra_driver_data` of the `init` call.
    ...
    /// HTML5 -- WebAudio ScriptProcessorNode output. Default on HTML5 if AWN isn't available.
